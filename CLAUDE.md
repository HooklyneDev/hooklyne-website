...
- EN + NL versions on every page (inline pattern via `useLang`, no i18n library). **Every change ships EN + NL in the same commit. Both written natively. No translation passes.** See `DOCS/nl-voice.md` for the native Dutch voice guide (Mollie/AFAS/TOPdesk-inspired) - read it before writing any NL copy. Drop calque structures (`tot we groeien`, `leest alsof`, `het meeste van die jaren`, `X is wat je krijgt als je Y`). Use period-heavy short sentences, fragments OK, drop hedges. See `DOCS/i18n-sync-tracker.md` for which pages are paired.
