import { useState } from "react";
import { Check, ArrowRight } from "lucide-react";
import { cn } from "@/lib/utils";

type Plan = {
 name: string;
 sub: string;
 monthly: number | null;
 annual: number | null;
 priceLabel?: string;
 features: string[];
 highlighted?: boolean;
 cta: string;
};

const PLANS: Plan[] = [
 {
 name: "Start",
 sub: "Solo reps and founders.",
 monthly: 59,
 annual: 53,
 features: ["75 credits / mo", "1 seat", "Email digest signals", "Weekly sector news", "Self-serve onboarding"],
 cta: "Get started",
 },
 {
 name: "Growth Smarter",
 sub: "Teams with consistent outbound.",
 monthly: 149,
 annual: 134,
 features: ["150 credits / mo", "2 seats", "Real-time signals", "10 discovery / mo", "Onboarding call", "€1.20 overage"],
 highlighted: true,
 cta: "Get started",
 },
 {
 name: "Scale",
 sub: "Full teams, higher volume.",
 monthly: 349,
 annual: 314,
 features: ["500 credits / mo", "5 seats", "Real-time signals", "Unlimited discovery", "Full setup", "€0.90 overage"],
 cta: "Get started",
 },
 {
 name: "Enterprise",
 sub: "Sales departments, multi-market.",
 monthly: null,
 annual: null,
 priceLabel: "Custom",
 features: ["Custom credit volume", "Unlimited seats", "Real-time signals", "Unlimited discovery", "Full setup + training", "Custom rate"],
 cta: "Talk to sales",
 },
];

export const HomePricing = () => {
 const [annual, setAnnual] = useState(true);

 return (
 <section className="py-20 lg:py-28">
 <div className="container max-w-6xl">
 <div className="text-center max-w-2xl mx-auto mb-10">
 <h2 className="text-3xl md:text-4xl font-semibold text-[var(--heading)] tracking-tight mb-4">
 Pricing
 </h2>
 <p className="text-base text-[var(--muted-foreground)]">Switch plans or cancel anytime.</p>
 </div>

 <div className="flex items-center justify-center gap-3 mb-10">
 <span className={cn("text-sm font-medium", !annual ? "text-[var(--heading)]" : "text-[var(--muted-foreground)]")}>Monthly</span>
 <button
 onClick={() => setAnnual(!annual)}
 className="relative w-12 h-6 rounded-full transition-colors"
 style={{ background: "var(--hooklyne-navy)" }}
 aria-label="Toggle billing"
 >
 <span
 className="absolute top-0.5 left-0.5 size-5 rounded-full bg-white transition-transform"
 style={{ transform: annual ? "translateX(24px)" : "translateX(0)" }}
 />
 </button>
 <span className={cn("text-sm font-medium flex items-center gap-1.5", annual ? "text-[var(--heading)]" : "text-[var(--muted-foreground)]")}>
 Annual
 <span className="text-xs font-semibold text-[var(--hooklyne-teal)] bg-[var(--hooklyne-teal)]/10 px-1.5 py-0.5 rounded-full">Save 10%</span>
 </span>
 </div>

 <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
 {PLANS.map((p) => {
 const price = p.priceLabel ?? `€${annual ? p.annual : p.monthly}`;
 return (
 <div
 key={p.name}
 className={cn(
 "rounded-2xl p-5 flex flex-col relative bg-[var(--card)]",
 p.highlighted && "ring-2 ring-[var(--hooklyne-navy)]",
 )}
 style={{ boxShadow: p.highlighted ? "var(--shadow-lg)" : "var(--shadow-xs)" }}
 >
 {p.highlighted && (
 <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[var(--hooklyne-navy)] text-white text-xs font-semibold px-3 py-1 rounded-full whitespace-nowrap">
 Recommended
 </div>
 )}
 <p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted-foreground)] mb-3">{p.name}</p>
 <div className="mb-1">
 <span className="text-3xl font-bold text-[var(--heading)]">{price}</span>
 {p.monthly != null && <span className="text-sm text-[var(--muted-foreground)] ml-1">/mo</span>}
 </div>
 <p className="text-xs text-[var(--muted-foreground)] mb-5">{p.sub}</p>
 <ul className="flex-1 space-y-2 mb-6 text-sm">
 {p.features.map((f) => (
 <li key={f} className="flex items-start gap-2 text-[var(--muted-foreground)]">
 <Check className="size-4 text-[var(--hooklyne-teal)] shrink-0 mt-0.5" />
 {f}
 </li>
 ))}
 </ul>
 <a
 href="/contact"
 className={cn(
 "block text-center rounded-lg text-sm font-semibold py-2.5 transition-colors btn-shine",
 p.highlighted
 ? "bg-[var(--hooklyne-navy)] text-white hover:opacity-90"
 : "ring-1 ring-[var(--hooklyne-navy)] text-[var(--heading)] hover:bg-[var(--hooklyne-navy)] hover:text-white",
 )}
 >
 {p.cta}
 </a>
 </div>
 );
 })}
 </div>

 <div className="text-center mt-8">
 <a href="/pricing" className="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--hooklyne-blue)] hover:opacity-80 transition-opacity">
 See credit breakdown and full pricing
 <ArrowRight className="size-3.5" />
 </a>
 </div>
 </div>
 </section>
 );
};
