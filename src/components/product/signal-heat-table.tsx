    ...
    <GraphicShell crumb="Portal / Prospect Signals" status="Live" statusTone="teal" ratio="3/2">
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) {
          .sh-row, .sh-bar, .sh-pulse { animation: none !important; }
      ...
      <div className="absolute inset-0 flex flex-col px-4 py-3 sm:px-6 sm:py-5">
        ...
        <div className="rounded-lg p-2 sm:p-2.5 mb-2 sm:mb-3 grid grid-cols-3 gap-2 sm:gap-3" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          ...
          <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 border-l sm:border-l pl-2 sm:pl-3" style={{ borderColor: "var(--border)" }}>
            ...
            <div className="min-w-0">
              ...
              <p className="text-[9px] sm:text-[10px]" style={{ color: "var(--muted-foreground)" }}>Hottest · 38.0</p>
            </div>
          </div>
        ...
        <div className="flex-1 min-h-0 rounded-lg overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          ...
          <div className="flex flex-col">
            {ROWS.map((r, i) => (
              <div
                key={r.company}
                className={`sh-row grid items-center px-2.5 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-[11px] ${i < ROWS.length - 1 ? "border-b" : ""}`}
                style={{
